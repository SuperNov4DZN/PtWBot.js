...
module.exports.run = async (bot, message, args) => {
    ...
    if (member.highestRole.position >= message.member.highestRole.position) return message.reply("Você não pode mutar alguem com um cargo maior que você!");
    ...
    if (member.roles.has(role.id)) return message.reply(`O usuario ${member.user.tag} já está mutado!`);
